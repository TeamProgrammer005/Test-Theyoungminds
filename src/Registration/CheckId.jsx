import React from 'react';
import "./registration.css";
import { useState, useEffect } from 'react';
import { useParams } from 'react-router-dom';
import ym_watermark from "../image/ymwatermark.png";
import QrCode from 'react-qr-code'
import { BsCalendarCheck } from "react-icons/bs";
import { FaWhatsapp } from "react-icons/fa"
import {  AiOutlineInstagram } from "react-icons/ai";
import { GoLocation } from "react-icons/go"
import { getTData } from '../api';
import {MdCheckCircle,MdDangerous}  from 'react-icons/md'

const Checkid = () => {
    const  [compclass, setCompclass]= useState(1);
    const [data, setData]=useState('')
    const {UniqueCode} = useParams();
    // const range1= '2022-01-19'
    // const range2= '2022-01-21'
    const getBytoken = async()=>{
      let  res= await getTData(UniqueCode)
      
   
      if (res.data[0] !== undefined) {
    
          setData(res.data[0])
          setCompclass(2)
      } else {
        setCompclass(3)
      }
    if (compclass===2) {
          const nowDateTime= new Date()
          const startTime=new Date(data.product.locations[0].startDateTime) ;
          const endTime=new Date(data.product.locations[0].endDateTime);
          console.log(nowDateTime);
        if (nowDateTime<=startTime || 
            nowDateTime<=endTime || 
            (nowDateTime>=startTime && nowDateTime<=endTime)) {
            let B=  document.getElementById('unVerify').style
            B.display='none'
    
        } else {
            let B=  document.getElementById('unVerify').style
          let A= document.getElementById('verify').style
          A.display='none'
          B.display='block'
        }
    }


    }

    useEffect(() => {
        getBytoken()
    }, );
 
    if (compclass === 2) {
        return (
            <div className='putItcenter' id="pdf1" >
                <div className="container workshopform">
                    <form className="thankyoumsg toplabel" >
                        <div className='thankmsg'>Thank You for joining us.</div>
                        <div className="conForm" id="pdf4" >
                            <div className="conForm2" id='pdf5'>
                               
                                <div className='makeroundShadow mx-4 my-2'> <div className="rowDisplay"><MdCheckCircle size={135} color="rgb(95 183 17)" id='verify'/><MdDangerous size={135} color="red" id='unVerify' /></div> </div>
                                <div className='qrname mb-2'>{data.FirstName} {data.LastName} </div>
                                <div id='pdf6' className="conForm1">  <QrCode fgColor='#193566' value=""  height={50} size={90} /></div>
                                <div className='ticketnumber'> {data.UniqueCode} </div>
                                <hr style={{ width: "100%" }} />
                                <div className='qrlocation'> <GoLocation className='mr-2' />{data.product.locations[0].name}</div>
                                <hr style={{ width: "100%" }} />
                                <div className='qrlocation'><BsCalendarCheck className='mr-2' />{data.product.locations[0].startDateTime.split('T')[0]}</div>
                                <hr style={{ width: "100%" }} className='mb-2' />
                                <div>
                                    <img className="ymimage mt-2" src={ym_watermark} alt="logo" />
                                    <div style={{ fontWeight: "bold" }} className='mb-2 ymname'>theyoungminds.org</div>
                                </div>
                            </div>
                        </div>
                        <div className='shareicons'>
                            
                            <div className='allicons'> <a href='https://wa.me/+918910540012'><FaWhatsapp size="2.5rem" color='#193566' className='mr-4 sicon' /></a> </div>
                            <div className='allicons'>
                                <a href='https://www.instagram.com/youngminds.education/?hl=en'>
                                <AiOutlineInstagram size="2.5rem" color='#193566' className='mr-4 sicon' />
                                </a></div>
                            <div className='allicons'>
                            <a href='https://www.facebook.com/theyoungminds.org'>
                                <img className='sicon' width={"40px"} alt="svgImg" src="data:image/svg+xml;base64," />
                                </a>
                            </div>
                        </div>
                    </form>
                </div>
            </div>
        )
    } else if (compclass === 1) {
        return (
            <div className='putItcenter' >
                <div className="spinner-grow" role="status">
                    <span className="visually-hidden">Loading...</span>
                </div>
            </div>
        )
    }if (compclass === 3) {
        return (
            <div className='putItcenter' >
                    <h1>Page not found 404</h1>
            </div>
        )
    }
   
}

export default Checkid;
